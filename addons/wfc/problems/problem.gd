...
func pick_divergence_option_at(options: Array[int], _cell: int) -> int:
	return pick_divergence_option(options)

func compute_entropy_at(domain: WFCBitSet, _cell: int) -> int:
	var bits := domain.count_set_bits()
	return bits - 1
