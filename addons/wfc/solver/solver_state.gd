...
func set_domain(cell_id: int, domain: WFCBitSet, entropy: int = -1) -> bool:
	...
	if only_bit == WFCBitSet.ONLY_BIT_NO_BITS_SET:
		...
		if entropy < 0:
			entropy = problem.compute_entropy_at(domain, cell_id)
...
func diverge() -> WFCSolverState:
	...
	var solution := problem.pick_divergence_option_at(divergence_options, divergence_cell)
...
func ensure_ac4_state(binary_constraints: Array[WFCProblem.AC4BinaryConstraint]):
